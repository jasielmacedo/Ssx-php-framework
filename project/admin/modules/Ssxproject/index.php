 ...
 */
 ...
 $args = array();
  ...
  $query = array(
  	'AND'=>array(
  		array('field'=>$SsxGroups->prefix.'.level','compare'=>'!=','value'=>"0"),
  		array('field'=>$SsxGroups->prefix.'.level','compare'=>'!=','value'=>2),
  		array('field'=>$SsxGroups->prefix.'.deleted','compare'=>'=','value'=>"0"),
  	)
  );
  $groups = $SsxGroups->getAll($SsxGroups->prefix . '.name', 'ASC',0,0,$query);
  ...
  $args['fields'][] = array('field'=>'project_pages_allow','label'=>'Permitir exibição de Páginas para projetos', 'type'=>'check','options'=>array('1'=>'Ativar'));
  
  $args_v = array(
  		'fields'=>array(
  				array('field'=>'label','type'=>'label','label'=>'SEO do Site'),
  				array('field'=>'project_seo_title','label'=>'T&iacute;tulo do Site', 'type'=>'text'),
  				...
  				array('field'=>'label3','type'=>'label','label'=>'Configuração de envio de email'),
  				array('field'=>'project_use_send_grid','label'=>'Usar Servi&ccedil;o SendGrid&copy; para enviar email', 'type'=>'check','options'=>array('1'=>'Ativar')),
  				...
  				array('field'=>'project_smtp_port','label'=>'SMTP Server Port', 'type'=>'text','mask'=>'9?9999'),
  		)
  );
  
  $args['fields'] = array_merge($args['fields'],$args_v['fields']);
  
  $args = SsxActivity::dispatchActivity('ssx_project_edit_args', $args);
  
  $SsxEditConstruct = new SsxEditConstruct($args);
 
  // recuperando dados //
  $fields_value = array(
  	'project_use_send_grid'=>SsxConfig::get(SsxConfig::SSX_USE_SEND_GRID),
 ...
 if($SsxEditConstruct->save())
 {
 	 ...
 	 SsxConfig::set(SsxConfig::SSX_USE_SEND_GRID,$data_request['project_use_send_grid']);
 	 ...
 	 SsxActivity::dispatchActivity('ssx_project_edit_save', $data_request);
