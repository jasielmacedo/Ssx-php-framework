...
class SsxAjax
{
	...
	private function importAjaxFile($class_prefix)
	{
		if(!$class_prefix)
		  return;
		...
		}else if(file_exists(LOCALPATH . "../control/ajax/".$class_prefix.".php"))
		{
			require_once(LOCALPATH . "../control/ajax/".$class_prefix.".php");
			return true;
		}else if(file_exists(COREPATH . "library/classes/ajax/".$class_prefix.".php"))
		{
			require_once(COREPATH . "library/classes/ajax/".$class_prefix.".php");
			return true;
		}else{
	...
	private function executeAction()
	{
		...
		$class_prefix = preg_replace("/(\\.)|(\\/)|(\\\\)/is", "", $class_prefix);
		...
		{			
			...
			if(class_exists($class_prefix))
			{
				...
				if($obj && $obj instanceof SsxAjaxElement)
				{
					// verifica as permissoes de acesso para ajax
					if(SsxAcl::checkPermissionForAction($obj->module,$obj->action,$obj->platform))
					{
						...
						var_dump($obj->module,$obj->action,$obj->platform);
