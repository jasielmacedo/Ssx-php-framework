...
function SsxJs()
{
	...
	this.functionsCallBacks = [];
	
	this.ajax = function(function_to_call, data, callback, errorCallback)
	{
		...
		this.functionsCallBacks.push(callback);
		
		var dataToSend = {
			'function_call' : function_to_call,
			'function_data' : data,
			'function_callback' : this.functionsCallBacks.length,
			'output' : 'json',
			'ad' : (ad)?true:false
		};
		...
		$.ajax({
			  ...
			  success : function(data)
			  {
					...
					if(data.callback)
					{
						try
						{
							if (Ssx.isFunction(Ssx.functionsCallBacks[data.callback-1])) 
							{
								Ssx.functionsCallBacks[data.callback-1](data.result);
							}else
							{
								if(navigator.appName.indexOf("Microsoft") != -1)
								{
									document[Ssx.functionsCallBacks[data.callback-1]](data.result);
								}else{
									window[Ssx.functionsCallBacks[data.callback-1]](data.result);
								}
								
								Ssx.functionsCallBacks[data.callback-1] = null;
							}
						}catch(e)
						{
							console.log('function callback have problems: '+e);
						}	
					}
			  },
			  error : errorFunction
	...
	this.confirm = function(callback,text)
	{
		...
		{
			var boxDialog = "" +
				"<div id='ssx_dialog_confirm' class='modal fade' tabindex='-1' role='dialog' aria-hidden='true'>" +
	...
	this.alert = function(text,callback)
	{
		...
		{
			var boxDialog = "" +
				"<div id='ssx_dialog_alert' class='modal fade' tabindex='-1' role='dialog' aria-hidden='true'>" +
				  "<div class='modal-dialog'>"+
				    "<div class='modal-content'>"+
						"<div class='modal-header'>"+
				    		"<button type='button' class='close' data-dismiss='modal'><span aria-hidden='true'>&times;</span><span class='sr-only'>Close</span></button>"+
		...
		jQuery(form).find(".required:visible").each(function(index){
			...
			switch(type)
			{
				case "email":
					if(val == "")
					{
						jQuery(error_point).html("Informe o email");
						Ssx.validateSubmit = 0;
					}else if(!Ssx.isEmail(val))
					{
						jQuery(error_point).html("Email Inválido");
						Ssx.validateSubmit = 0;
					}else{
				...
				case "cpf":
					if(!Ssx.isCpf(val))
					{
						jQuery(error_point).html("Cpf inválido");
						Ssx.validateSubmit = 0;
					}else{
						jQuery(error_point).html("");
					}
				...
				case "pass":
					...
					if(val == "")
					{
						jQuery(error_point).html("Preencha o campo corretamente");
						Ssx.validateSubmit = 0;
					}else if(val.length < min_length)
					{
						jQuery(error_point).html("A senha precisa ter pelo menos "+min_length+" caracteres");
						Ssx.validateSubmit = 0;
					}else if(compare)
					{
						if(val != jQuery(compare).val())
						{
							jQuery(error_point).html("Senhas não conferem");
							Ssx.validateSubmit = 0;
						}else{
							jQuery(error_point).html("");
						}
				...
				default:
					...
					}else if(jQuery(this).is('input')) 
					{
						...
						{
							jQuery(error_point).html("O Campo precisa ter pelo menos "+min_length+" caracteres");
							Ssx.validateSubmit = 0;
						}else{
							jQuery(error_point).html("");
						}
